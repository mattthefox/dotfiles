...
class RemoveUnusedVertexGroups(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in bpy.context.selected_objects:
            ...
            # Add .R groups back to any existing groups.
            for vg in obj.vertex_groups:
                if vg.name.endswith(".L"):
                    rightGroup = vg.name.replace(".L","") + ".R"
                    if rightGroup not in obj.vertex_groups:
                        obj.vertex_groups.new(name=rightGroup)
                        self.report({"INFO"}, f"Re-added {rightGroup}")
