...
def audio_update(self, context, use_all=False):
    ...
    filter_complex += f"; {mix_inputs}amix=inputs={input_file_index}:dropout_transition=0[aout]"
...
class ZEPHKIT_PT_LoopToolsPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		# NLA quick tools
		layout.label(text="Tweak All", icon="NLA")
		row = layout.row(align=True)
		enterall = row.operator("zephkit.tweak_mode_all", text="Enter", icon="TRIA_UP").enter = True
		exitall = row.operator("zephkit.tweak_mode_all", text="Exit", icon="TRIA_DOWN").enter = False


		layout.separator()
		# Update menu
		layout.label(text="Update...", icon="FILE_REFRESH")
		row = layout.row(align=True)
		row.operator("zephkit.update_intensity", text="Intensity", icon="IPO_ELASTIC")
		row.operator('zephkit.render_loops_audio',text="Audio",icon="FILE_SOUND")
		row.operator("zephkit.global_update", text="All", icon="FILE_REFRESH")
		layout.separator()
		...
		if active_action:
			...
			if not active_loop_index == -1:
				...
				time_row = row.split(align=True,factor=0.4)
				time_row.prop(loop_props, "intensity_variation", text="Var.")
				...
				layout.prop(loop_props,"audio_volume",text="Volume %")

				layout.prop(loop_props, "rest_frame", text="Resting frame")
