bl_info = {
    ...
    'category': 'UI',
    'doc_url': "https://www.pragma37.com/theme_editor",
    'tracker_url': "https://www.pragma37.com/contact"
}
...
def set_collection_length(collection, length):
    while len(collection) < length: collection.add()
    while len(collection) > length: collection.remove(len(collection) - 1)
...
def sort_paths(search_terms):
    for color, params in colors.items():
        params.sort(key=lambda e : (filter_string(e.path, search_terms) == False, e.path))
        ...
        def inspect_struct(struct, path=""):
            for key in dir(struct):
                ...
                if issubclass(type(attribute), bpy.types.bpy_struct):
                    ...
                    color = color_to_tuple(attribute)
                    if color:
                        theme_properties.append(ColorTuple(struct,key,path+separator+name))
...
def property_index_callback(self, context):
    ...
    if self.highlight_selected:
        set_color(color_paths(group_index)[last_property_index], highlight_color)
        set_color(color_paths(group_index)[new_index], property_highlight_color)
...
class ColorGroupProperties(bpy.types.PropertyGroup):
    ...
    color : bpy.props.FloatVectorProperty(name="Color", size=4, subtype='COLOR_GAMMA', min=0.0, max=1.0, update=color_updated)
...
class WM_OT_theme_edit_execute_preset(bpy.types.Operator):
    ...
    def execute(self, context):
        global last_theme
        last_theme = None
        bpy.ops.script.execute_preset(filepath=self.filepath, menu_idname="THEME_EDIT_MT_Presets")
        return {'FINISHED'}
...
class WM_OT_theme_edit_save_current_preset(bpy.types.Operator):
    bl_idname = "wm.theme_edit_save_current_preset"
    bl_label = "Save Current Theme Preset"
    bl_description = "Save the current theme"

    def execute(self, context):
        current_theme = bpy.types.THEME_EDIT_MT_Presets.bl_label
        file_name = AddPresetBase.as_filename(current_theme) + '.xml'
        target_path = os.path.join("presets", "interface_theme")
        ...
        rna_xml.xml_file_write(context, file_path, preset_xml_map)
...
class VIEW_3D_PT_theme_editor(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("wm.theme_edit_save_current_preset", text="", icon='FILE_TICK')
        row.operator("wm.theme_edit_interface_theme_preset_add", text="", icon='DUPLICATE')
        #row.operator("wm.theme_edit_interface_theme_preset_add", text="", icon='TRASH').remove_active = True
        row.operator("wm.theme_edit_open_preset_folder", text="", icon='FILE_FOLDER')
        ...
        if colors is not None:
            ...
            layout.template_list("VIEW_3D_UL_color_list","",theme_edit,"color_groups",theme_edit,"group_index")
            ...
            if "theme_edit" in bpy.data.palettes:
                ...
                layout.template_palette(theme_edit,"history_palette",color=True)
        else:
            box = layout.box()
...
class WM_OT_edit_theme_colors(bpy.types.Operator):
    ...
    def run_implementation(self, context):
        ...
        for group in theme_edit.color_groups:
            ...
            set_color_group(group.index, (color.r, color.g, color.b, group.color[3]))

    
    hue : bpy.props.FloatProperty(name="Hue",min=-1,max=1,update=run_implementation)
    saturation : bpy.props.FloatProperty(name="Saturation",min=-1,max=1,update=run_implementation)
    value : bpy.props.FloatProperty(name="Value",min=-1,max=1,update=run_implementation)
    contrast : bpy.props.FloatProperty(name="Contrast",min=-1,max=1,update=run_implementation)
    invert: bpy.props.BoolProperty(name="Invert colors", update=run_implementation)
...
the ones with lower user counts are merged into the ones with higher user counts"""
    ...
    h : bpy.props.FloatProperty(name="Hue", description="Hue Tolerance", default=0.015, min=0.0015, max=1.0, update=run_implementation)
    s : bpy.props.FloatProperty(name="Saturation", description="Saturation Tolerance", default=0.015, min=0.0015, max=1.0, update=run_implementation)
    v : bpy.props.FloatProperty(name="Value", description="Value Tolerance", default=0.015, min=0.0015, max=1.0, update=run_implementation)
    a : bpy.props.FloatProperty(name="Alpha", description="Alpha Tolerance", default=0.015, min=0.0015, max=1.0, update=run_implementation)
...
Example: theme space, ? text, ? title, -highlight""", 
    ...
    group_index : bpy.props.IntProperty(name="Group Index", update=group_index_callback)
    color_paths : bpy.props.CollectionProperty(type=ColorPathsPropertyGroup)
    paths_index : bpy.props.IntProperty(default = 0, update=property_index_callback)
...
classes=[
    ...
    WM_OT_theme_edit_save_current_preset,
...
def register():
    ...
    bpy.types.WindowManager.theme_edit = bpy.props.PointerProperty(type=ThemeEditPropertyGroup)
